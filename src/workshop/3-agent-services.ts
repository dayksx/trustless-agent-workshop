...
// TODO: import { paymentMiddleware } from "x402-express";
...
// TODO: Add x402 paymentMiddleware for /paid-service (see workshop-correction/3-agent-services.ts)
// const payTo = (process.env.PAY_TO_ADDRESS as `0x${string}`) || AGENT2_SA_ADDRESS;
// app.use(paymentMiddleware(payTo, { "/paid-service": { price: "$0.01", network: "base-sepolia", ... } }));
...
// x402 payable endpoint — requires paymentMiddleware (not configured yet)
app.post("/paid-service", async (req, res) => {
  console.log("💰 POST /paid-service:", req.body.message);
  throw new Error("TODO: Implement x402 payment for /paid-service (add paymentMiddleware)");
});
